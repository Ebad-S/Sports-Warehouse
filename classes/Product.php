...
class Product
{
  ...
  public function getProduct($id)
  {
    try {
      ...
      $sql = <<<SQL
          SELECT  itemId, itemName, price, salePrice
          FROM    item
          WHERE   itemId = :itemId
        SQL;
  ...
  public function updateProduct($itemId)
  {
    try {
      ...
      if (!($fileUploadOptional && $_FILES["photoPath"]["error"] === "UPLOAD_ERR_NO_FILE")) {

        // Get the filename of the uploaded file
        $fileName = basename($_FILES["photoPath"]["name"]);
        ...
        if (!in_array($fileExtension, $validExtensions)) {
          ...
          $errors["photoPath"] = "Invalid file extension, must be: " . implode(", ", $validExtensions);
        }

        // Check file size
        // The uploaded file exceeds the 
        if (
          $_FILES["photoPath"]["error"] === UPLOAD_ERR_INI_SIZE ||
          $_FILES["photoPath"]["error"] === UPLOAD_ERR_FORM_SIZE
        ) {
          // File too big
          $errors["photoPath"] = "File is too large.";
        }
        if (empty($errors)) {
          ...
          $moveFrom = $_FILES["photoPath"]["tmp_name"];
          ...
          if (move_uploaded_file($moveFrom, $moveTo)) {
            ...
            // Error
            $errors["photoPath"] = "Uploaded file could not be moved.";
          }
        }
      }
      if ($_FILES["photoPath"]["error"] !== UPLOAD_ERR_NO_FILE) {
